...
class SecureDatabaseClient:
    ...
    @contextmanager
    def get_transaction_client(self, operation_type: str = "gui"):
    ...
    def validate_security_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = anon_client.table("departments").select("department_id").limit(1).execute()
        ...
        try:
            ...
            result = service_client.table("departments").select("department_id").limit(1).execute()
    ...
    def test_rls_policies(self) -> Dict[str, Any]:
        ...
        try:
            anon_client = self._get_anon_client()
            response = anon_client.table("departments").select("department_id").limit(1).execute()
            results["anon_access"]["allowed"] = len(response.data) >= 0  # Even empty result is success
        ...
        try:
            service_client = self._get_service_client()
            response = service_client.table("departments").select("department_id").limit(1).execute()
    ...
    def emergency_disable_rls(self) -> str:
        ...
        """
        if not security_config.validate_configuration().get("allow_emergency_disable", False):
            raise RuntimeError("Emergency RLS disable not allowed in current configuration")
...
@contextmanager
def database_transaction(operation_type: str = "gui"):
