...
class ApplicationSecurityManager:
    ...
    def authorize_action(
        ...
        if security_config.security_level in [SecurityLevel.DEPARTMENT, SecurityLevel.TENANT]:
    ...
    def _log_security_event(
        ...
        # Log security events
        logger.info(f"Security Event: {action.value} on {resource} by {context.user_id or 'anonymous'}")
        
        # Log access denied events as warnings
        if action == SecurityAction.ACCESS_DENIED:
            logger.warning(f"Access denied: {context.user_id or 'anonymous'} attempted {action.value} on {resource}")
    ...
    def check_rate_limit(self, user_id: str, max_attempts: int = 100, window_minutes: int = 60) -> bool:
        ...
        if current_attempts >= max_attempts:
            logger.warning(f"Rate limit exceeded for user {user_id}: {current_attempts} attempts")
            return False
...
def authorize_action(
    ...
    Returns:
        True if authorized
    """
    return security_manager.authorize_action(context, action, resource, resource_id, metadata)
