...
class SecurityMiddleware:
    ...
    async def __call__(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            self._log_request(auth_request, response.status_code, time.time() - start_time)
            ...
            if self.config.fail_gracefully:
                ...
                response = await call_next(request)
                response.headers["X-Security-Warning"] = "Security check bypassed due to error"
                return response
            else:
                return JSONResponse(
                    status_code=500,
    ...
    async def _apply_security_checks(self, auth_request: AuthenticatedRequest) -> dict[str, Any]:
    ...
    def _can_use_service_role_escalation(self, auth_request: AuthenticatedRequest) -> bool:
    ...
    async def _validate_auth_token(self, token: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not jwt_secret:
                logger.warning("SUPABASE_JWT_SECRET not configured, using development mode")
    ...
    def _add_security_headers(self, response: Response, auth_request: AuthenticatedRequest) -> None:
    ...
    def _log_request(self, auth_request: AuthenticatedRequest, status_code: int, duration: float) -> None:
        ...
        if status_code >= 400:
            logger.warning(f"Request failed: {auth_request.method} {auth_request.path}", extra=log_data)
        else:
            logger.info(f"Request completed: {auth_request.method} {auth_request.path}", extra=log_data)
