...
class TemplateValidator:
    ...
    def __init__(self) -> None:
        ...
        self.validation_history: dict[str, list[TemplateValidationReport]] = {}

    def validate_template_comprehensive(
        self,
        template_id: str,
        job_template: Any,  # JobTemplate - avoiding circular import
        ...
        structure_issues = self._validate_template_structure(template_id, job_template)
        issues.extend(structure_issues)

        # Performance validation
        perf_issues, perf_metrics = self._validate_template_performance(
            template_id, job_template, test_instances, performance_baseline
        )
    ...
    def _validate_template_structure(
        self, template_id: str, job_template: Any
        ...
        if (
            not hasattr(job_template, "template_tasks")
            or not job_template.template_tasks
        ):
            issues.append(
                ValidationIssue(
                    severity="error",
        ...
        if not hasattr(job_template, "template_precedences"):
            issues.append(
                ValidationIssue(
                    severity="warning",
                    category="structure",
        ...
        if hasattr(job_template, "template_tasks"):
            for task in job_template.template_tasks:
                if not hasattr(task, "modes") or not task.modes:
                    issues.append(
                        ValidationIssue(
                            ...
                            category="structure",
                            message=(
                                f"Template task {task.template_task_id} has no "
                                "execution modes"
                            ),
                            template_id=template_id,
                            details={"task_id": task.template_task_id},
                        )
                    )
                    ...
                    if max(durations) - min(durations) > 480:  # 8 hours difference
                        issues.append(
                            ValidationIssue(
                                ...
                                category="structure",
                                message=(
                                    f"Template task {task.template_task_id} has large "
                                    "duration variance"
                                ),
                                template_id=template_id,
                                details={
        ...
        if hasattr(job_template, "template_precedences"):
            precedence_count = len(job_template.template_precedences)
            task_count = (
                len(job_template.template_tasks)
                if hasattr(job_template, "template_tasks")
                else 0
            )
    ...
    def _validate_template_performance(
        self, template_id: str, job_template: Any, test_instances: int, baseline: float
    ...
    def _validate_template_compatibility(
        self, template_id: str, job_template: Any  # noqa: ARG002
    ...
    def validate_template_regression(
        ...
        Returns:
            TemplateValidationReport focusing on regression analysis
    ...
    def _record_validation(self, report: TemplateValidationReport) -> None:
