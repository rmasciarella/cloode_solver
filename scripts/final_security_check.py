...
class FinalSecurityCheck:
    ...
    def check_critical_fixes(self):
        ...
        if db_file.exists():
            ...
            if re.search(r'f["\'].*SELECT.*FROM.*\{.*\}', content):
                ...
                if "get_database_client" in content or "supabase" in content.lower():
                    self.passed_checks.append("SQL injection fixed with parameterized queries")
        ...
        if rls_migrations:
            self.passed_checks.append(f"RLS migration files exist ({len(rls_migrations)} files)")
        ...
        for file in jwt_files:
            if Path(file).exists():
                ...
                if "jwt" in content.lower() and ("verify" in content or "decode" in content):
                    ...
                    try:
                        ...
                        for pattern in secret_patterns:
                            if re.search(pattern, content):
                                ...
                                self.failed_checks.append(f"Hardcoded secret in {filepath}")
                                break
                    except:
                        pass
