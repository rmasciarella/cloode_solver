...
class SecurityMigrationManager:
    ...
    def __init__(self):
        ...
        self.migration_file = (
            project_root / "migrations" / "003_add_auth_and_rls_foundation.sql"
        )
    ...
    def validate_prerequisites(self) -> dict[str, Any]:
        ...
        try:
            ...
            response = (
                client.table("departments").select("count", count="exact").execute()
            )
        ...
        try:
            ...
            machine_count = (
                client.table("machines").select("count", count="exact").execute()
            )

            total_records = (dept_count.count or 0) + (machine_count.count or 0)
            if total_records > 0:
                results["warnings"].append(
                    f"Existing data found: {total_records} records"
                )
    ...
    def backup_database(self) -> bool:
        ...
        try:
            ...
            # Save backup to file
            backup_file = (
                project_root / f"backups/migration_backup_{int(time.time())}.json"
            )
            ...
            for i, command in enumerate(sql_commands):
                ...
                try:
                    ...
                    if len(command) > 100:  # Complex command
                        logger.info(
                            f"Executing migration command {i+1}/{len(sql_commands)}"
                        )
    ...
    def test_post_migration(self) -> dict[str, Any]:
        ...
        try:
            ...
            response = (
                client.table("job_optimized_patterns").select("*").limit(1).execute()
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Validate without applying"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force migration even with warnings"
    )
    ...
    parser.add_argument(
        "--skip-tests", action="store_true", help="Skip post-migration tests"
    )
        ...
        if args.rollback:
            ...
            if success:
                ...
                results["recommendations"].append(
                    "Test GUI functionality after rollback"
                )
            ...
            if not args.skip_tests:
                ...
                if not post_test["gui_functionality"]["basic_operations"]["passed"]:
                    logger.error(
                        "Post-migration GUI test failed - considering rollback"
                    )
                    results["recommendations"].append(
                        "GUI functionality failed - manual investigation required"
                    )
            ...
            if security_config.security_level.value == "permissive":
                results["recommendations"].append(
                    "Consider upgrading to 'authenticated' security level"
                )
