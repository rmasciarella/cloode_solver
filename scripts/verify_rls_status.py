        ...
        try:
            ...
            for table in tables:
                try:
                    ...
                    anon_client = create_client(url, os.getenv('SUPABASE_ANON_KEY'))
                    try:
                        anon_result = anon_client.table(table).select('*').limit(1).execute()
                        print(f"⚠️  Anonymous can access {table} (permissive RLS or disabled)")
                    except:
...
def main():
    ...
    print(f"RLS Status:               {'✅ Enabled' if rls_ok else '❌ Not fully enabled'}")
    print("-" * 50)
