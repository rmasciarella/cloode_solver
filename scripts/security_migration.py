...
class PolicyDefinition:
    ...
    using_clause: str
    with_check_clause: str | None = None
...
class SecurityMigrator:
    ...
    def apply_security_level(
        self, level: SecurityLevel, tables: list[str] | None = None
    ):
        """Apply security policies for the specified level"""
        target_tables = tables or self._get_all_tables()

        print(
            f"Applying {level.value} security level to {len(target_tables)} tables..."
        )
    ...
    def _drop_table_policies(self, table: str):
        ...
        for policy in policies:
            ...
            if policy_name:
                try:
                    self.client.rpc(
                        "drop_policy", {"table_name": table, "policy_name": policy_name}
                    )
                except Exception as e:
    ...
    def validate_migration(self, tables: list[str] | None = None) -> bool:
...
def main():
    ...
    parser.add_argument(
        "level",
        choices=["permissive", "restricted", "strict"],
        help="Security level to apply",
    )
    parser.add_argument("--tables", nargs="+", help="Specific tables to migrate")
    parser.add_argument(
        "--validate", action="store_true", help="Validate after migration"
    )
